...
class Program:
    ...
    def __init__(self, text, llm=None, cache_seed=0, logprobs=None, silent='auto', async_mode=False, stream=None, caching=None, await_missing=False, update_callback=None, **kwargs):
        ...
        update_callback : Callable[[str], None] (default None)
            If set, this callback will be invoked each time an update is available for rendering. It should be a function
            that accepts a single string parameter, which will be the updated text of the program.
        """
    ...
    def _update_display(self, last=False):
        ...
        if self.update_callback:
            self.update_callback(self.marked_text)
